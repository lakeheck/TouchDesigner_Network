//average-frag.glsl
//post averaging to smooth sharp edges on terrain

// uniform sampler2D read_Terrain;
...
void main() {
    ...
    vec4 topright = texture(sTD2DInputs[0],curuv+vec2(div,div));
    vec4 right = texture(sTD2DInputs[0],curuv+vec2(div,0.f));
    vec4 bottomright = texture(sTD2DInputs[0],curuv+vec2(div,-div));
    vec4 bottom = texture(sTD2DInputs[0],curuv+vec2(0.f,-div));
    vec4 bottomleft = texture(sTD2DInputs[0],curuv+vec2(-div,-div));
    vec4 left = texture(sTD2DInputs[0],curuv+vec2(-div,0.f));
    vec4 topleft = texture(sTD2DInputs[0],curuv+vec2(-div,div));
