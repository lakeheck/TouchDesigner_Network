...
// uniform sampler2D sTD2DInputs[0] / hight_map;
// uniform sampler2D sTD2DInputs[1] / nor_map;
// uniform sampler2D sTD2DInputs[2] / sedi_map;
// uniform sampler2D sTD2DInputs[3] / vel_map;
// uniform sampler2D sTD2DInputs[4] / flux_map;
// uniform sampler2DsTD2DInputs[5] / terrainflux_map;
// uniform sampler2D sTD2DInputs[6] / maxslippage_map;
// uniform sampler2D sTD2DInputs[7] / sediBl_end;
// uniform sampler2D sTD2DInputs[8] / shadow_Map;
...
float computeTerrainAO(){
    vec4 HC = texture(sTD2DInputs[0],vUV.st);
    return 1.0;
}
...
void main()
{
    ...
    for(int x = -1; x <= 1; ++x)
    {
        for(int y = -1; y <= 1; ++y)
        {
            float pcfDepth = texture(sTD2DInputs[8], shadowMapLoc.xy + vec2(x, y) * texsize).r;
    ...
    shadowVal/=9.0;
    shadowCol/=9.0;
    float shadowColorVal = texture(sTD2DInputs[8], vUV.st.xy).x;
    ...
    if(u_BrushType != 0){
        ...
        float pdis2fragment = distance(pointOnPlane, vUV.st);
    ...
    vec3 slopesin = texture(sTD2DInputs[1],vUV.st).xyz;
    vec3 nor = -calnor(vUV.st);
    ...
    //lamb =1.f;
    vec4 fH = texture(sTD2DInputs[0],vUV.st);
    float yval = fH.x * 4.0;
    float wval = fH.y;
    float sval = texture(sTD2DInputs[7], vUV.st).x;
    ...
    if(u_TerrainDebug == 0){
        ...
        fcol = texture(sTD2DInputs[2],vUV.st).xyz * 2.0;
    }else if(u_TerrainDebug == 2){
        fcol = abs(texture(sTD2DInputs[3],vUV.st).xyz/20.0);
    }else if(u_TerrainDebug == 9){

        //fcol = vec3(length(texture(sTD2DInputs[3],vUV.st).xyz)/5.0);

        float velSize = length(texture(sTD2DInputs[3],vUV.st).xyz) / 5.0;
        ...
        fcol = texture(sTD2DInputs[0],vUV.st).xyz;
        fcol.xy /= 200.0;
        fcol.y *= 80.0;
        //fcol = vec3(fcol.z);
    }else if(u_TerrainDebug == 4){
        fcol = texture(sTD2DInputs[4],vUV.st).xyz / 3.0;
        if(fcol == vec3(0.0)){
            fcol = vec3(texture(sTD2DInputs[4],vUV.st).w)/3.0;
        }
    }else if(u_TerrainDebug == 5){
        fcol = texture(sTD2DInputs[5], vUV.st).xyz * 100000.0;
    }else if(u_TerrainDebug == 6){
        fcol = texture(sTD2DInputs[6], vUV.st).xyz / 13.0;
...
//    vec3 deepSedimentCol = vec3(0.4, 0.2, 0.0);
    ...
    if(!debug){
        ...
        if(u_SedimentTrace == 0){
            float ssval = texture(sTD2DInputs[2], vUV.st).x;
